...
class MomentumSlider {
  ...
  startDecelAnim() {
    ...
    // With loop: wrap position into valid range before animating
    if (this.o.loop) {
      const paddingLength = this.o.loop * this.step;
      const contentLength = this.sliderWidth - paddingLength * 2;
      while (-newTargetPosition < paddingLength) newTargetPosition += contentLength;
      while (-newTargetPosition >= paddingLength + contentLength) newTargetPosition -= contentLength;
    }
